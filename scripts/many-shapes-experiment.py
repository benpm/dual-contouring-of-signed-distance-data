...
def load_objs(data_dir="data/many-shapes-experiment"):
    """
    ...
    assert len(objs) == 10, "Not all .obj files were loaded. Expected 10."
    return objs[2:]
...
def run_large_scale_test(max_num_shapes=20, out_csv=None, rng=None, seed=42, res=50,
    ...
    for idx in tqdm(range(count)):
        ...
        # Write gt mesh to results folder
        # gt does not depend on res, but this makes our life easier when processing files later
        mesh_path = ROOT / 'results' / 'many-shapes-experiment' / f"{name}_res{res}_gt.obj"
        gpy.write_mesh(str(mesh_path), V_gt, F_gt)
        shape_tests = []
        mesh_list = []
        ...
        # Save the objs
        objs_dir = ROOT / 'results' / 'many-shapes-experiment'
        for V_mesh, F_mesh, shortname in mesh_list:
    ...
    metrics = ['chamfer', 'hdff', 'ecd_L2', 'ecd_L1', 'ef1', 'sdf_energy', 'n_verts']
    best_method = {
...
if __name__ == '__main__':
    ...
    # out = str(ROOT / 'results' / 'many-shapes-experiment' / 'results_50.csv')
    # run_large_scale_test(max_num_shapes=50, out_csv=out, rng=rng, seed=seed, res=50)

    # out = str(ROOT / 'results' / 'many-shapes-experiment' / 'results_100.csv')
    # run_large_scale_test(max_num_shapes=50, out_csv=out, rng=rng, seed=seed, res=100)

    out = str(ROOT / 'results' / 'many-shapes-experiment' / 'results_150.csv')
