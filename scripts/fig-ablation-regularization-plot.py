...
V_gt, _, _, F_gt, _, _ = igl.readOBJ(str(ROOT / "results" / "ablation-regularization" / f"{prefix}_gt.obj"))
V_dc, _, _, F_dc, _, _ = igl.readOBJ(str(ROOT / "results" / "ablation-regularization" / f"{prefix}_dc.obj"))
V_mnm1, _, _, F_mnm1, _, _ = igl.readOBJ(str(ROOT / "results" / "ablation-regularization" / f"{prefix}_mnm1.obj"))
V_mnm2, _, _, F_mnm2, _, _ = igl.readOBJ(str(ROOT / "results" / "ablation-regularization" / f"{prefix}_mnm2.obj"))
V_rfta, _, _, F_rfta, _, _ = igl.readOBJ(str(ROOT / "results" / "ablation-regularization" / f"{prefix}_rfta.obj"))
...
mu_list = [0.001, 0.01, 0.1, 1.0, 10.0, 100.0, 1000.0]
...
for mu in mu_list:
    V, _, _, F, _, _ = igl.readOBJ(str(ROOT / "results" / "ablation-regularization" / f"{prefix}_mu{mu}_ours.obj"))
    V_ours[mu] = V
    F_ours[mu] = F
    ...
    print(f"Ours mu={mu}: Chamfer distance = {chamfer_dist:.6e}, Hausdorff distance = {hdff_dist:.6e}")
...
ax.set_xlabel('Regularization weight (mu)')
...
# Save figure as eps
plt.savefig(os.path.join(ROOT, 'results/ablation-regularization/plot_ablation_regularization.eps'), format='eps')
plt.savefig(os.path.join(ROOT, 'results/ablation-regularization/plot_ablation_regularization.png'), format='png', dpi=300)
plt.show()