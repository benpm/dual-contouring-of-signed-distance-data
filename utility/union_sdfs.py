...
def union_sdfs(sdf1, sdf2, res=48, min_corner=-1.0, max_corner=1.0, use_dc_true=False):
    """
    Create a SDF that is the union of two SDFs.

    Args:
        sdf1 (SDFObject): The first signed distance function object.
        sdf2 (SDFObject): The second signed distance function object.
        res (int, optional): Resolution used to extract the union mesh. Defaults to 48.
    Returns:
        SDFObject: A new SDFObject representing the union of the two input SDFs.
        V (ndarray): Vertices of the extracted mesh.
        F (ndarray): Faces of the extracted mesh.
    """

    union_sdf = contouring._contouring_cpp_module.SDFObject()
    ...
    def min_func(p):
        return min(sdf1.f(p), sdf2.f(p))
    
    # Evaluate the min function on the grid
    S = np.array([min_func(p) for p in GV])
    ...
    union_sdf.f = f
    union_sdf.grad = finite_difference_gradient(union_sdf.f, h=1e-6)

    return union_sdf, V, F
