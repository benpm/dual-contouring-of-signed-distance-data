...
results_path = "results/ablation-dc-weight/"
...
for dc_weight in dc_weight_list:
    ...
    gpy.write_mesh( results_path + f"{mesh}_dc{dc_weight}_ours.obj", verts_ours @ np.linalg.inv(R), faces_ours)
    V_ours[dc_weight] = verts_ours
    F_ours[dc_weight] = faces_ours
...
# Show ours
for dc_weight in dc_weight_list:
    ps.register_surface_mesh(f"ours-dc{dc_weight}", V_ours[dc_weight] @ np.linalg.inv(R), F_ours[dc_weight])
ps.show()