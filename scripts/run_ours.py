...
results_path = "results/run_ours/"
if not os.path.exists(results_path):
    os.makedirs(results_path)
gs = 100 # grid size
mesh = "01_00040053_fdbd1ff80ef645bca6474bad_trimesh_000"
...
print(f"  - mnm1: {mnm1_time:.4f} seconds")
print(f"  - mnm2: {mnm2_time:.4f} seconds")
...
ps.register_surface_mesh("mnm2", V_mnm2 @ np.linalg.inv(R), F_mnm2, enabled=False)
ps.register_surface_mesh("rfta", V_rfta @ np.linalg.inv(R), F_rfta, enabled=False)
ps.register_surface_mesh("mc", verts_mc @ np.linalg.inv(R), faces_mc, enabled=False)
ps.show()