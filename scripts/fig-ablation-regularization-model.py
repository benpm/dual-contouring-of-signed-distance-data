...
# Source: https://www.thingiverse.com/thing:421017
mesh = "Empire_State"
gs = 100
...
results_path = "results/ablation-regularization/"
...
for mu in mu_list:
    ...
    gpy.write_mesh( results_path + f"{mesh}_mu{mu}_ours.obj", verts_ours @ np.linalg.inv(R), faces_ours)
    V_ours[mu] = verts_ours
    F_ours[mu] = faces_ours
...
# Show ours
for mu in mu_list:
    ps.register_surface_mesh(f"ours-mu{mu}", V_ours[mu] @ np.linalg.inv(R), F_ours[mu])
ps.show()